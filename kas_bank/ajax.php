...
if ($parameter == 1) {
	...
	$query[1] = mysqli_query($conn, "UPDATE counter SET digit='$kode' WHERE tabel='no_kas_keluar'");
	...
	$saldo = $qsaldo['saldo_jalan'];
	$saldoInsert = $saldo + $jumlah;
	$query[1] = mysqli_query($conn, "UPDATE counter SET digit='$kode' WHERE tabel='no_kas_masuk'");
	$query[2] = mysqli_query($conn, "UPDATE bank SET saldo_jalan='$saldoInsert' WHERE kode_bank='$kode_bank'");
	...
	$query = array();
	$sql = "UPDATE counter SET digit='$kode1' WHERE tabel='no_kas_keluar';";
	$sql .= "UPDATE counter SET digit='$kode2' WHERE tabel='no_kas_masuk';";
	...
	$tahun = date('Y');
	$totals = query("SELECT total FROM `sales_invoice` WHERE YEAR(CAST(created_at AS DATE))=$tahun AND kode_customer = '$kode_customer'");
	$data['total'] = 0;
